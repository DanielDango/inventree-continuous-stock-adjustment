...
import { LocalizedComponent } from './locale.tsx';
...
function ContinouousStockAdjustmentPanel({
    ...
    const apiQuery = useQuery(
        {
            ...
            queryFn: async() => {
                const url = `/plugin/continouous-stock-adjustment/example/`;
    ...
    return (
        ...
        {editPartForm.modal}
        <Stack gap="xs">
        <Title c={context.theme.primaryColor}  order={3}>Continouous Stock Adjustment</Title>
        <Text>
            This is a custom panel for the ContinouousStockAdjustment plugin.
        </Text>
        <SimpleGrid cols={2}>
...
export function renderContinouousStockAdjustmentPanel(context: InvenTreePluginContext) {
    checkPluginVersion(context);

    return (
        <LocalizedComponent locale={context.locale}>
            <ContinouousStockAdjustmentPanel context={context} />
        </LocalizedComponent>
    );
}
